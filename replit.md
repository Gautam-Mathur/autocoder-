...
## Recent Changes
-   **Feb 14, 2026**: Advanced stress testing (5 phases) and intent classifier fix. Phase 1: Edit accumulation (30+ edits) — no schema/API drift. Phase 2: Contradiction handling — contradictory edits handled gracefully. Phase 3: Partial failure — edge cases handled without crashes. Phase 4: Error-to-chat loop — no infinite recursion, history capped at 50. Phase 5: Learning drift — stable scores (85-90 avg, improvement trend 1.2) across 20 pipeline runs. Fixed intent classifier: added compound style keywords (text color, font color, background color, border color, font family, font size, font weight) with 3-point scoring, and skip-logic to prevent "text" content keyword from overriding when style compound detected. Edit success rate: ~60% (style/color/size edits work, structural modifications limited by regex-based system).
-   **Feb 14, 2026**: Fixed 6 critical bugs from stress testing. BUG A: Entities now get domain-specific fields (price/sku/email/status) via learning engine inference. BUG B: FK alias system resolves cross-entity references (userId→customers, assigneeId→teams) with 10+ alias mappings. BUG C: Archetype scoring uses matchCount multiplier for correct domain matching (hospital→Healthcare, learning→LMS, social→Social Network). BUG D: CSS `--background`/`--foreground` variable edits, multi-word page targeting regex, generic page resolution (main/home/index). BUG E: Action words filtered from entity names. Also fixed searchSimilar to handle category="all" without WHERE filter.
